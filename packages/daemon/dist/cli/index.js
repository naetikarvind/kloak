...
async function callDaemonIpc(method, params = {}) {
    return new Promise((resolve, reject) => {
        const tryConnect = (useSocket) => {
            const client = useSocket
                ? net.createConnection(socket_server_js_1.SOCKET_PATH)
                : net.createConnection(socket_server_js_1.TCP_PORT, socket_server_js_1.TCP_HOST);
            let responseBuffer = '';
            client.on('connect', () => {
    ...
    switch (command) {
        ...
        case 'get': {
            ...
            try {
                ...
                let liveTotp = item.totpSecret ? (0, core_1.generateTotp)(item.totpSecret) : undefined;
                res = { item, liveTotp };
            }
            const item = res.item;
        ...
        case 'add': {
            ...
            if (!password) {
                password = (0, core_1.generatePassword)({ length: 20 });
        ...
        case 'generate': {
            ...
            if (isPassphrase) {
                ...
                const phrase = (0, core_1.generatePassphrase)({ wordsCount: words });
                const strength = (0, core_1.evaluatePasswordStrength)(phrase);
                ...
                const pwd = (0, core_1.generatePassword)({ length });
                const strength = (0, core_1.evaluatePasswordStrength)(pwd);
        ...
        case 'totp': {
            ...
            try {
                const totp = (0, core_1.generateTotp)(secretOrQuery);
                ...
                if (matches.length && matches[0].totpSecret) {
                    const totp = (0, core_1.generateTotp)(matches[0].totpSecret);
        ...
        case 'export': {
            ...
            let exportPass;
            let format = isEncrypted ? 'kloak-encrypted' : isCsv ? 'kloak-csv' : isBitwarden ? 'bitwarden-json' : 'kloak-json';
            if (isEncrypted) {
        ...
        case 'install-manifests': {
            ...
            const paths = host_js_1.NativeMessagingHost.installManifests();
