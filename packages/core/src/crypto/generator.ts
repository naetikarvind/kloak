...
export function generatePassword(options: PasswordGeneratorOptions = {}): string {
  ...
  const selectedPools: { pool: string; required: number }[] = [];
  if (useUpper && upperPool.length > 0) selectedPools.push({ pool: upperPool, required: options.minUppercase ?? 1 });
  if (useLower && lowerPool.length > 0) selectedPools.push({ pool: lowerPool, required: options.minLowercase ?? 1 });
  if (useNumbers && numberPool.length > 0) selectedPools.push({ pool: numberPool, required: options.minNumbers ?? 1 });
  if (useSymbols && symbolPool.length > 0) selectedPools.push({ pool: symbolPool, required: options.minSymbols ?? 1 });
...
export function evaluatePasswordStrength(password: string): PasswordStrengthResult {
  ...
  if (password.length < 10) suggestions.push('Make it at least 12-16 characters long.');
