...
function analyzeFormContext(focusedInput: HTMLInputElement): FormContext {
  ...
  const submitButtons = Array.from(container.querySelectorAll('button[type="submit"], input[type="submit"]')) as HTMLElement[];
...
function injectCredentials(username?: string, password?: string, targetInput?: HTMLInputElement) {
...
function createDropdownUI(items: any[], rect: DOMRect) {
  if (activeDropdown) {
      ...
      row.addEventListener('click', () => {
        ...
        if (context.isEmailFirst) {
          ...
          const observer = new MutationObserver((mutations, obs) => {
            const passwordInputs = (Array.from(document.querySelectorAll('input[type="password"]')) as HTMLInputElement[]).filter(isVisible);
  ...
  const closeOnClickOutside = (e: MouseEvent) => {
    if (!dropdown.contains(e.target as Node) && e.target !== currentFocusedInput) {
      dropdown.remove();
      activeDropdown = null;
...
function showDropdown(input: HTMLInputElement) {
...
function showThreatBanner(analysis: any, connectedAccount?: any) {
...
function showSavePasswordPrompt(username: string, password: string) {
  chrome.runtime.sendMessage({
...
function showSuccessNotification(customMsg?: string) {
