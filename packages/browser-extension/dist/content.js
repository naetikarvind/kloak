...
function attachKloakFieldBadge(input) {
...
const onClickOutside = (ev) => {
    if (currentPopup && !currentPopup.contains(ev.target) && ev.target !== currentTargetInput) {
        closePopup();
    }
};
const onKeydown = (ev) => {
...
const onScrollDismiss = (ev) => {
    if (currentPopup && !currentPopup.contains(ev.target)) {
        closePopup();
    }
};
function updatePopupPosition() {
    if (!currentPopup || !currentTargetInput)
        return;
...
async function showAutofillMenu(targetInput) {
...
function setupFormSubmissionDetector() {
    ...
    document.addEventListener('input', (e) => {
        ...
        if (target.type === 'password' && target.value) {
            ...
            if (form) {
                const userInp = form.querySelector('input[type="text"], input[type="email"], input[name*="user"], input[name*="login"]');
                if (userInp && userInp.value)
                    lastUsername = userInp.value;
            }
            else {
                ...
                if (visibleUserInp)
                    lastUsername = visibleUserInp.value;
            }
        }
    }, true);
    const checkAndPrompt = () => {
        if (!lastPassword || lastPassword.length < 4)
            return;
...
function showSavePasswordPrompt(details) {
